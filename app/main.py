...
from app.core.config import SECRET_KEY, ALGORITHM, ALLOWED_ORIGINS
from app.core.database import Base, engine, get_db
from app.core.security import (
    create_access_token, get_current_user,
    staff_only, admin_only, log_audit
)
...
from app.routers import (
    admin, clearance, business, inspections,
    audit, requirements, reports,
    settings, forgot_password, archive, users
)
...
def logout(
    ...
    log_audit(db, current_user.id, "LOGOUT", "AUTH", None, {"email": current_user.email, "full_name": current_user.full_name})
...
def get_business_lines(db: Session = Depends(get_db)):
    setting = db.query(SystemSetting).filter(
        SystemSetting.key == "business_lines"
    ).first()
