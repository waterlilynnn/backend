...
def _active_violation_labels(violations: dict) -> list:
    if not violations:
        return []
    return [VIOLATION_LABELS.get(k, k) for k, v in violations.items() if v]


def _get_inspection_frequency(db: Session) -> dict:
    setting = db.query(SystemSetting).filter(
        SystemSetting.key == "inspection_frequency"
    ).first()
    if not setting or not setting.value:
...
def can_inspect_business(
    ...
    """
    Check if a business can be inspected based on configured frequency.
    """
    ...
    freq_config = _get_inspection_frequency(db)
    frequency   = freq_config.get("frequency", 1)
    period      = freq_config.get("period", "year")

    last_inspection = (
        db.query(Inspection)
        .filter(Inspection.business_record_id == record_id)
    ...
    if period == "year":
        if last_date.year == now.year:
            ...
            can = count_this_year < frequency
            return {
                "can_inspect":          can,
                "reason":               None if can else f"Maximum {frequency} inspection(s) per year reached",
    ...
    delta = period_delta_map.get(period)
    if delta is None:
        return {"can_inspect": True, "reason": None}

    if last_date + delta <= now:
        return {
            "can_inspect":          True,
            "reason":               None,
            "inspection_count":     0,
        ...
        return {
            "can_inspect":          False,
            "reason":               f"Maximum {frequency} inspection(s) per {period} reached",
            "inspection_count":     1,
            "max_count":            frequency,
            "period":               period,
            "last_inspection_date": last_date.isoformat(),
...
def submit_inspection_checklist(
    ...
    if _is_business_line_exempted_from_inspection(db, record.business_line):
        raise HTTPException(
            status_code=400,
            detail=f"Business line '{record.business_line}' is exempted from inspections.",
        )
    ...
    if period == "year":
        ...
        try:
            ...
            delta = delta_map.get(period, relativedelta(years=1))
            count_this_period = (
                db.query(Inspection)
                .filter(
                    Inspection.business_record_id == record_id,
                    Inspection.inspection_date >= now - delta,
                )
                .count()
            )

    if count_this_period >= frequency:
        raise HTTPException(
            status_code=400,
            detail=f"This business has already been inspected {count_this_period} time(s) this {period}. "
    ...
    summary_text      = (
        payload.get("summary_other")
        if payload.get("summary") == "Other"
    ...
    log_audit(
        db, current_user.id, "INSPECT", "BUSINESS", record_id,
        {
            "status":       status.value,
            "inspection_id": inspection.id,
...
def resolve_inspection(
    ...
    if days_since > 15:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Cannot resolve violation. The 15-day resolution period has passed "
                f"(inspection was {days_since} days ago)."
            ),
        )
...
def create_inspection(
    ...
    log_audit(
        db, current_user.id, "INSPECT", "BUSINESS", record_id,
        {"status": status.value, "inspection_id": inspection.id},
    )
...
def get_inspection_checklist(
    ...
    cl = (
        ...
        .filter(InspectionChecklist.inspection_id == inspection_id)
        .first()
    )
    if not cl:
...
def get_all_inspections(
    ...
    for i in inspections:
        ...
        result.append({
            ...
            "resolved_remarks":  i.resolved_remarks,
            "violations": (
                _active_violation_labels(i.checklist.violations)
                if i.checklist and i.checklist.violations
                else []
            ),
        })
    return result
